...
const LmsKuAdmin = ({ bankSoal, setoran, pengaturan, daftarUjian, keLogin, emailAdmin, superAdmin }) => {
  ...
  // 👈 FIX: DOWNLOAD EXCEL RAPI MENGGUNAKAN FORMAT HTML TABLE KE XLS
  const unduhExcel = () => {
    const tableHtml = `
        ...
        <table border="1">
          <thead>
            <tr>
              ...
              <th style="background-color: #f1f5f9;">Nama Siswa</th>
              <th style="background-color: #f1f5f9;">Email (ID)</th>
              ${ujianKelasIni.map(u => `<th style="background-color: #e0e7ff;">${u.judul}</th>`).join('')}
              <th style="background-color: #d1fae5;">Skor Total</th>
            </tr>
          </thead>
          <tbody>
            ${rekapRapor.map((s, idx) => `
              <tr>
                <td>${idx+1}</td>
                <td>${s.nama}</td>
                <td>${s.email}</td>
                ${ujianKelasIni.map(u => `<td>${s.nilaiPerUjian[u.docId] || 0}</td>`).join('')}
                <td style="font-weight: bold;">${s.totalSkor}</td>
              </tr>
            `).join('')}
          </tbody>
        </table>
    ...
    link.href = url;
    link.download = `Rapor_Kelas_${kelasAktif}.xls`;
  ...
  const handleSimpanSoal = async (e) => {
    ...
    try {
      ...
      setForm(prev => ({ 
         ...prev, teksSoal: '', teksTambahanArab: '', opsiA: '', opsiB: '', opsiC: '', opsiD: '', opsiE: '', 
         kunci: prev.tipe.includes('pilihan_ganda') ? [] : '', mediaSoalGambar: null, mediaSoalSuara: null 
      }));
      setEditId(null);
  ...
  const salinSoal = (soal) => {
     ...
     setEditId(null); window.scrollTo({ top: 0, behavior: 'smooth' });
  };
  ...
  return (
    <div className="p-4 md:p-8 font-sans max-w-7xl mx-auto pb-32">
      ...
      {qrHalaqah && (
         <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
            <div className="bg-white dark:bg-slate-800 p-8 rounded-3xl shadow-2xl flex flex-col items-center max-w-sm w-full relative">
               <button onClick={() => setQrHalaqah(null)} className="absolute top-4 right-4 bg-red-100 text-red-600 w-8 h-8 rounded-full font-bold hover:bg-red-500 hover:text-white">✕</button>
      ...
      {/* 👈 TAB MENU ADMIN YANG KEMBALI UTUH */}
      <div className="flex gap-2 overflow-x-auto pb-4 mb-6 border-b border-slate-200 dark:border-slate-700">
          <button onClick={() => {setTabAdmin('buat'); setSetoranTerpilih(null);}} className={`px-4 py-2 font-bold rounded-lg text-sm transition-all ${tabAdmin === 'buat' ? 'bg-indigo-100 text-indigo-700 dark:bg-indigo-900/50 dark:text-indigo-300' : 'text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800'}`}>➕ Buat Jadwal & Soal</button>
          <button onClick={() => {setTabAdmin('forum'); setSetoranTerpilih(null);}} className={`px-4 py-2 font-bold rounded-lg text-sm transition-all ${tabAdmin === 'forum' ? 'bg-sky-100 text-sky-700 dark:bg-sky-900/50 dark:text-sky-300' : 'text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800'}`}>💬 Forum Kelas</button>
          <button onClick={() => {setTabAdmin('koreksi'); setSetoranTerpilih(null);}} className={`px-4 py-2 font-bold rounded-lg text-sm transition-all ${tabAdmin === 'koreksi' ? 'bg-orange-100 text-orange-700 dark:bg-orange-900/50 dark:text-orange-300' : 'text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800'}`}>✅ Evaluasi ({setoranTampil.length})</button>
          <button onClick={() => {setTabAdmin('rapor'); setSetoranTerpilih(null);}} className={`px-4 py-2 font-bold rounded-lg text-sm transition-all ${tabAdmin === 'rapor' ? 'bg-teal-100 text-teal-700 dark:bg-teal-900/50 dark:text-teal-400' : 'text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800'}`}>📊 Data Siswa & Rapor</button>
          <button onClick={() => {setTabAdmin('peringkat'); setSetoranTerpilih(null);}} className={`px-4 py-2 font-bold rounded-lg text-sm transition-all ${tabAdmin === 'peringkat' ? 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/50 dark:text-yellow-400' : 'text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800'}`}>🏆 Peringkat</button>
          {isSuperAdmin && (
      ...
      {/* 👈 FITUR RAPOR, EXCEL, DAN TAMBAH/KELUARKAN MURID */}
         ...
         <div className="space-y-6">
            ...
            <div className="bg-white dark:bg-slate-800 p-6 md:p-8 rounded-[2.5rem] shadow-sm border border-slate-100 dark:border-slate-700 transition-colors">
               <div className="flex justify-between items-center mb-6">
                  <h2 className="text-2xl font-black text-slate-800 dark:text-white tracking-tight">📊 Buku Rapor & Daftar Siswa Aktif</h2>
                  <button onClick={unduhExcel} className="bg-emerald-500 hover:bg-emerald-600 text-white px-4 py-2 rounded-xl font-bold text-xs shadow-md transition-colors">📥 Unduh Excel</button>
               </div>
                ...
                <div className="space-y-3">
                   {!(pengaturan.daftarGuru && pengaturan.daftarGuru.length > 0) ? (
                      ...
                      pengaturan.daftarGuru.map((email, idx) => (
                         <div key={idx} className="flex justify-between items-center bg-white dark:bg-slate-800 p-3 md:p-4 rounded-2xl border border-slate-100 dark:border-slate-600 shadow-sm transition-colors gap-2">
                            <div className="flex items-center gap-3 overflow-hidden">
                               <div className="w-8 h-8 md:w-10 md:h-10 shrink-0 bg-purple-100 dark:bg-purple-900/40 text-purple-600 dark:text-purple-400 rounded-xl flex items-center justify-center font-black text-base md:text-lg">{email.charAt(0).toUpperCase()}</div>
                               <p className="font-bold text-slate-700 dark:text-white truncate text-sm md:text-base">{email}</p>
                            </div>
                            <button onClick={() => hapusGuru(email)} className="text-[10px] md:text-xs font-bold text-red-500 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300 bg-red-50 dark:bg-red-900/30 px-3 py-2 rounded-lg transition-colors shrink-0">Cabut Akses</button>
                         </div>
                      ))
                   )}
                </div>
      ...
      {/* 👈 FITUR KELOLA JADWAL DAN SOAL UJIAN (BUAT) */}
      {tabAdmin === 'buat' && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-7 space-y-4">
            <div className={`bg-white dark:bg-slate-800 p-6 rounded-3xl shadow-sm border-2 h-fit transition-colors ${editId ? 'border-orange-400 dark:border-orange-500 bg-orange-50 dark:bg-orange-900/10' : 'border-slate-100 dark:border-slate-700'}`}>
              <div className="flex justify-between items-center mb-4">
                 ...
                 {editId ? (
                    ...
                    <button onClick={() => window.print()} className="bg-slate-100 dark:bg-slate-700 px-3 py-1 rounded-lg text-xs font-bold text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors shadow-sm">📄 Cetak Soal / Word</button>
                 )}
              </div>
            ...
            <div className="max-h-[600px] overflow-y-auto pr-2 custom-scrollbar space-y-4 mt-6 no-print">
            ...
            {/* 👈 TAMPILAN CETAK WORD YANG TERSEMBUNYI (HANYA MUNCUL SAAT DI-PRINT) */}
            <div className="hidden print:block font-serif text-black space-y-4 mt-8">
               <h1 className="text-xl font-bold text-center mb-8 border-b-2 border-black pb-4 uppercase">NASKAH UJIAN: {ujianKelasIni.find(u => u.docId === ujianAktifAdmin)?.judul || 'SOAL'}</h1>
               {soalTampil.map((soal, idx) => (
      ...
      {/* 👈 FITUR KOREKSI/EVALUASI YANG KEMBALI */}
      ...
      {tabAdmin === 'peringkat' && halaqahMilikGuru.length > 0 && (
        <div className="bg-white dark:bg-slate-800 p-6 md:p-8 rounded-[2.5rem] shadow-sm border border-slate-100 dark:border-slate-700 min-h-[50vh] transition-colors">
          ...
          <div className="grid grid-cols-1 gap-8">
             <div className="bg-slate-50 dark:bg-slate-700 p-6 rounded-3xl border-2 border-slate-100 dark:border-slate-600 transition-colors">
               <h3 className="text-lg font-black text-indigo-600 dark:text-indigo-400 uppercase mb-4 tracking-widest text-center">🏆 Top Peringkat Ujian Ini</h3>
               
               {setoranTampil.length === 0 ? (
                  <p className="text-center text-xs text-slate-400 dark:text-slate-500 italic">Belum ada murid yang selesai.</p>
               ) : (
                  <div className="space-y-3">
                    {[...setoranTampil].sort((a, b) => b.nilaiSistem - a.nilaiSistem || (a.waktuPengerjaan || 9999) - (b.waktuPengerjaan || 9999)).slice(0, 10).map((murid, idx) => (
